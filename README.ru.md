...
PI_WEB_PASSWORD='длинный-случайный-пароль' pi-web  # пароль шлюза приложения в форке
PI_WEB_NO_OPEN=1 pi-web         # удобно для фоновой службы
```

В этом форке используется fail-closed шлюз приложения со страницей входа, подписанными сессиями и маршрутами `/api/gate/*`; он не связан со входом к поставщикам моделей. Если пароль отсутствует, конфигурация неверна или файл не найден, приложение остаётся заблокированным. Шлюз отключается только явно через `PI_WEB_AUTH_DISABLED=true` (или `auth.disabled: true` в файле конфигурации).

Pi Web может вызывать агента с широкими полномочиями. Для удалённого доступа оставляйте шлюз включённым и используйте HTTPS через доверенный обратный прокси или надёжное VPN-подключение.
