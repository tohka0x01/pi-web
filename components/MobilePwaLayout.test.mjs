...
const workspaceFilePanelSource = await readFile(new URL("./WorkspaceFilePanel.tsx", import.meta.url), "utf8");
...
test("tracks the visual viewport while the software keyboard is open", () => {
  ...
  assert.match(workspaceFilePanelSource, /height: "calc\(36px \+ env\(safe-area-inset-top\)\)"/);
  assert.match(workspaceFilePanelSource, /paddingTop: "env\(safe-area-inset-top\)"/);
